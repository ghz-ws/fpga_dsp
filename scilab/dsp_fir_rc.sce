...
subplot(2,4,1)
plot(coe,"o-")    //plot impulse response (also coefficient)
xgrid()
...
subplot(2,4,3)
plot(fr_nom,hm,"o-")  //plot freq response
xgrid()
title("Freq Response","fontsize",5)
...
subplot(4,4,4)
plot(X(1:L-TAP/2),I(1:L-TAP/2),"o-",X(1:L-TAP/2),I_OUT(TAP/2+1:L),"o-")
...
subplot(4,4,8)
plot(X(1:L-TAP/2),Q(1:L-TAP/2),"o-",X(1:L-TAP/2),Q_OUT(TAP/2+1:L),"o-")
legend("Input", "Output", 1)
xgrid()
...
subplot(2,2,3)
plot(f,20*log10(abs(y1(1:fn))),f,20*log10(abs(y2(1:fn))))
legend("Input", "Output", 1)
...
for i=1:(L-TAP)/rate/2
    plot(tv',I_OUT(rate*2*(i-1)+TAP:rate*2*(i-1)+TAP+rate*2),"o-")
end
xgrid()
...
subplot(2,4,8)
plot(I_OUT(TAP/2+1:L),Q_OUT(TAP/2+1:L))
xgrid()
